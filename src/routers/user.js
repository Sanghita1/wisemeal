...
router.post("/forgot", async (req,res)=>{
    ...
    const forgotUser = User.find({email}).then((result)=>{
        ...
        const token = jwt.sign({_id:result[0]._id, email:result[0].email}, SECRET, {expiresIn: "15m"});
...
router.post("/reset/:id/:token", (req,res)=>{
    ...
    // res.send({id,token,pass1,pass2});
})

router.patch("/reset", (req,res)=>{
    // const {id, token} = req.params;
    // const {pass1, pass2} = req.body;
    const {id, token, pass1, pass2} = req.body;
    ...
    // const SECRET = JWT_SECRET + forgotUser.password;
    // try{
    //     const payload = jwt.verify(token, SECRET);
    //     forgotUser.password = pass1;
    //     forgotUser.save();
